...
const UserList = () => {
  ...
  const getdataToDownload = async () => {
    ...
    const res = await fetch(
      `/api/getUsersToDownload?search=${searchTerm}&isActiveOrNot=${status}`,
      {
        method: "GET",
        headers: {
  ...
  React.useEffect(() => {
    ...
    const getdata = async () => {
      const res = await fetch(
        `/api/getUsers?page=${page}&search=${searchTerm}&isActiveOrNot=${status}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      {data ? (
        <div className="container list-section mt-4">
          ...
          <div className="d-flex justify-content-center align-items-center mb-4">
            ...
            >
              {" "}
              {page > Math.ceil(getUserData.total / 7) &&
              Math.ceil(getUserData.total) != 0
                ? setPage(1)
                : page}{" "}
              of {Math.ceil(getUserData.total / 7)}
            </p>
            <button
              disabled={page >= Math.ceil(getUserData.total / 7) ? true : false}
              className="btn"
              style={{ backgroundColor: "rgb(6, 0, 97)", color: "white" }}
