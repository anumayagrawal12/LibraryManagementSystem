...
const myIssuedBooks = (props) => {
  ...
  const getdataToDownload = async () => {
    ...
    const res = await fetch(
      `/api/getIssuedBookListToDownload?startDate=${startDate}&endDate=${endDate}&search=${searchTerm}&id=${
  ...
  useEffect(() => {
    ...
    const getdata = async () => {
      const res = await fetch(
        `/api/showIssuedBooksRequest?page=${page}&startDate=${startDate}&endDate=${endDate}&search=${searchTerm}&id=${
          props.userId ? props.userId : ""
        }&searchName=${searchUserName}`,
        {
          method: "GET",
      ...
      {data ? (
        <div className="container list-section mt-4">
                    ...
                    <tr className="record-row" key={serialNum}>
                      ...
                      {element.dateOfIssue ? (
                        <td style={{ color: "green" }}>
                          {element.dateOfIssue
                            ? element.dateOfIssue.slice(0, 10)
                            : ""}
                        </td>
                      ) : (
                      ...
                      {element.dateOfReturn ? (
                        <td
                          style={{ color: "green" }}
                        >{`${element.dateOfReturn.slice(0, 10)}`}</td>
                      ) : (
                      ...
                      <td className="d-flex justify-content-around">
                        <>
                          {element.dateOfIssue ? (
                            <>
                              <NavLink to={`editIssue/${element._id}`}>
                                <button
                                  className="btn mx-2"
                                  style={{
                              ...
                              <button
                                ...
                                disabled={
                                  element.dateOfReturn || disable ? true : false
                                }
                                onClick={() => bookReturn(element._id)}
                              >
                                Return
                              </button>
                            </>
                            ...
                            <button
                              ...
                              disabled={disable}
                              onClick={() => acceptRequest(element._id)}
                            >
                              Credit
                            </button>
                          )}
          ...
          <div className="d-flex justify-content-center align-items-center mt-4 mb-4">
            <button
              ...
              className="btn"
              style={{ backgroundColor: "rgb(6, 0, 97)", color: "white" }}
              onClick={() => {
                setPage(page - 1);
              }}
            >
            ...
            >
              {" "}
              {page > Math.ceil(getRequestData.total / 6) &&
              Math.ceil(getRequestData.total) != 0
                ? setPage(1)
                : page}{" "}
              of {Math.ceil(getRequestData.total / 6)}
            </p>
            <button
              disabled={
                page >= Math.ceil(getRequestData.total / 6) ? true : false
              }
              className="btn"
              style={{ backgroundColor: "rgb(6, 0, 97)", color: "white" }}
        ...
        <div className="m-auto">
          <div
            ...
            style={{ height: "4rem", width: "4rem", color: "rgb(6, 0, 97)" }}
            role="status"
          ></div>
        </div>
      )}
