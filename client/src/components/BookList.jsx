...
const BookList = () => {
  ...
  const getdataToDownload = async () => {
    ...
    const res = await fetch(
      `/api/getBooksToDownload?search=${searchTerm}&sortStock=${sortStock}`,
      {
        method: "GET",
        headers: {
  ...
  React.useEffect(() => {
    ...
    const getdata = async () => {
      const res = await fetch(
        `/api/getBooks?page=${page}&search=${searchTerm}&sortStock=${sortStock}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      {data ? (
        <div className="container list-section mt-4">
                    ...
                    <tr className="record-row" key={id}>
                      ...
                      <td className="d-flex justify-content-around">
                        <>
                          ...
                          <NavLink to={`addOn/${element._id}`}>
                            <button className="btn text-black">Add</button>
                          </NavLink>
                          <NavLink to={`bookIssueRequestForm/${element._id}`}>
                            <button
                              className="btn"
                              style={{
          ...
          <div className="d-flex justify-content-center align-items-center mb-4">
            ...
            >
              {" "}
              {page > Math.ceil(getBookData.total / 7) &&
              Math.ceil(getBookData.total) != 0
                ? setPage(1)
                : page}{" "}
              of {Math.ceil(getBookData.total / 7)}
            </p>
            <button
              disabled={page >= Math.ceil(getBookData.total / 7) ? true : false}
              className="btn"
              style={{ backgroundColor: "rgb(6, 0, 97)", color: "white" }}
